...
namespace UnityEngine.Rendering.PostProcessing
{
	...
	public sealed class TemporalAntialiasing
	{
		...
		{
			return SystemInfo.supportedRenderTargetCount >= 2 && SystemInfo.supportsMotionVectors && !RuntimeUtilities.isVREnabled && SystemInfo.graphicsDeviceType != GraphicsDeviceType.OpenGLES2;
		}
		...
		{
			Vector2 result = new Vector2(HaltonSeq.Get(m_SampleIndex & 0x3FF, 2), HaltonSeq.Get(m_SampleIndex & 0x3FF, 3));
			if (++m_SampleIndex >= 8)
			{
				m_SampleIndex = 0;
			}
		...
		public Matrix4x4 GetJitteredProjectionMatrix(Camera camera)
		{
			...
			jitter *= jitterSpread;
			Matrix4x4 result = ((jitteredMatrixFunc == null) ? ((!camera.orthographic) ? RuntimeUtilities.GetJitteredPerspectiveProjectionMatrix(camera, jitter) : RuntimeUtilities.GetJitteredOrthographicProjectionMatrix(camera, jitter)) : jitteredMatrixFunc(camera, jitter));
		...
		private RenderTexture CheckHistory(int id, PostProcessRenderContext context)
		{
			...
			if (m_ResetHistory || renderTexture == null || !renderTexture.IsCreated())
			{
				...
				renderTexture = RenderTexture.GetTemporary(context.width, context.height, 0, context.sourceFormat);
			...
			else if (renderTexture.width != context.width || renderTexture.height != context.height)
			{
				...
				GenerateHistoryName(temporary, id, context);
				temporary.filterMode = FilterMode.Bilinear;
				m_HistoryTextures[xrActiveEye][id] = temporary;
				context.command.BlitFullscreenTriangle(renderTexture, temporary);
				RenderTexture.ReleaseTemporary(renderTexture);
			}
