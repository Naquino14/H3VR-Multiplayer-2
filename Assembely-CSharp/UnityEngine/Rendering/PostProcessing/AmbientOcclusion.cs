...
namespace UnityEngine.Rendering.PostProcessing
{
	...
	public sealed class AmbientOcclusion : PostProcessEffectSettings
	{
		...
		{
			bool flag = enabled.value && intensity.value > 0f && !RuntimeUtilities.scriptableRenderPipelineActive;
			if (mode.value == AmbientOcclusionMode.MultiScaleVolumetricObscurance)
			{
				flag &= SystemInfo.supportsComputeShaders && SystemInfo.SupportsRenderTextureFormat(RenderTextureFormat.RFloat);
			}
			return flag;
		}
	}
