	...
	public sealed class PostProcessDebugLayer
	{
		...
		internal void RenderSpecialOverlays(PostProcessRenderContext context)
		{
			...
			else if (debugOverlay == DebugOverlay.Normals)
			{
				...
				PushDebugOverlay(context.command, BuiltinRenderTextureType.None, propertySheet, 1);
			}
			else if (debugOverlay == DebugOverlay.MotionVectors)
			{
				...
				propertySheet2.properties.SetVector(ShaderIDs.Params, new Vector4(overlaySettings.motionColorIntensity, overlaySettings.motionGridSize, 0f, 0f));
				PushDebugOverlay(context.command, context.source, propertySheet2, 2);
			}
			else if (debugOverlay == DebugOverlay.NANTracker)
			{
				...
				PushDebugOverlay(context.command, context.source, sheet2, 3);
			}
			else if (debugOverlay == DebugOverlay.ColorBlindnessSimulation)
			{
				...
				propertySheet3.properties.SetVector(ShaderIDs.Params, new Vector4(overlaySettings.colorBlindnessStrength, 0f, 0f, 0f));
				PushDebugOverlay(context.command, context.source, propertySheet3, (int)(4 + overlaySettings.colorBlindnessType));
			}
		}
