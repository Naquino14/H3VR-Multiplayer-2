...
namespace AmplifyBloom
{
	...
	public class AmplifyLensFlare : IAmplifyItem
	{
		...
		{
			m_lensFlareGradTexture = new Texture2D(256, 1, TextureFormat.ARGB32, mipmap: false);
		...
		public void TextureFromGradient()
		{
			for (int i = 0; i < 256; i++)
			{
				ref Color reference = ref m_lensFlareGradColor[i];
				reference = m_lensGradient.Evaluate((float)i / 255f);
			}
