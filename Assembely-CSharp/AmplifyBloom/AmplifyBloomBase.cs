...
namespace AmplifyBloom
{
	...
	public class AmplifyBloomBase : MonoBehaviour
	{
		...
		private void Awake()
		{
			...
			for (int i = 0; i < 6; i++)
			{
				ref Vector2 reference = ref m_tempDownsamplesSizes[i];
		...
		{
			InstaOld component = GetComponent<InstaOld>();
			if (component != null && component.enabled)
			{
				component.BindMaterial(m_finalCompositionMaterial);
				m_finalCompositionMaterial.EnableKeyword("INSTA_OLD");
			}
			else
			{
				m_finalCompositionMaterial.DisableKeyword("INSTA_OLD");
			}
