			...
			if (m_isDirty || m_currentWidth != source.width || m_currentHeight != source.height)
			{
				...
				for (int i = 0; i < m_glareMaxPassCount; i++)
				{
					...
					for (int j = 0; j < 8; j++)
					{
						...
						ref Vector4 reference = ref m_amplifyGlareCache.CromaticAberrationMat[i, j];
						reference = Color.Lerp(m_whiteReference, b, glareDefData.ChromaticAberration);
					}
				}
				...
				for (int l = 0; l < starDefData.StarlinesCount; l++)
				{
					...
					vector.x = num7 / num * (starLineData.SampleLength * m_overallStreakScale);
					vector.y = num6 / num2 * (starLineData.SampleLength * m_overallStreakScale);
					...
					for (int m = 0; m < num3; m++)
					{
						for (int n = 0; n < 8; n++)
						{
							...
							ref Vector4 reference2 = ref m_amplifyGlareCache.Starlines[l].Passes[m].Weights[n];
							reference2 = m_amplifyGlareCache.CromaticAberrationMat[num3 - 1 - m, n] * num9 * ((float)m + 1f) * 0.5f;
						...
						{
							ref Vector4 reference3 = ref m_amplifyGlareCache.Starlines[l].Passes[m].Offsets[num10];
							reference3 = -m_amplifyGlareCache.Starlines[l].Passes[m].Offsets[num10 - 8];
							ref Vector4 reference4 = ref m_amplifyGlareCache.Starlines[l].Passes[m].Weights[num10];
							reference4 = m_amplifyGlareCache.Starlines[l].Passes[m].Weights[num10 - 8];
						}
