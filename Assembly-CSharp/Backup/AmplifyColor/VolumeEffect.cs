...
namespace AmplifyColor
{
  ...
  public class VolumeEffect
  {
    ...
    public VolumeEffectComponent AddComponent(
      Component c,
      VolumeEffectComponentFlags compFlags)
    {
      if (compFlags == null)
      {
        VolumeEffectComponent volumeEffectComponent = new VolumeEffectComponent(c.GetType().ToString() + string.Empty);
        this.components.Add(volumeEffectComponent);
        return volumeEffectComponent;
      }
      VolumeEffectComponent effectComponent;
      if ((effectComponent = this.FindEffectComponent(c.GetType().ToString() + string.Empty)) != null)
      {
    ...
    public static Component[] ListAcceptableComponents(AmplifyColorBase go) => (UnityEngine.Object) go == (UnityEngine.Object) null ? new Component[0] : ((IEnumerable<Component>) go.GetComponents(typeof (Component))).Where<Component>((Func<Component, bool>) (comp => (UnityEngine.Object) comp != (UnityEngine.Object) null && ((comp.GetType().ToString() + string.Empty).StartsWith("UnityEngine.") ? 1 : (comp.GetType() == typeof (AmplifyColorBase) ? 1 : 0)) == 0)).ToArray<Component>();

    public string[] GetComponentNames() => this.components.Select<VolumeEffectComponent, string>((Func<VolumeEffectComponent, string>) (r => r.componentName)).ToArray<string>();
  }
}
