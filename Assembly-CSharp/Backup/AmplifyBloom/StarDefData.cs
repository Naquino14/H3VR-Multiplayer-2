...
namespace AmplifyBloom
{
  ...
  public class StarDefData
  {
    ...
    public void CalculateStarData()
    {
      ...
      float num = ((double) this.m_customIncrement <= 0.0 ? 180f / (float) this.m_starlinesCount : this.m_customIncrement) * ((float) Math.PI / 180f);
      for (int index = 0; index < this.m_starlinesCount; ++index)
      {
        ...
        this.m_starLinesArr[index].Attenuation = (double) this.m_longAttenuation <= 0.0 ? this.m_attenuation : (index % 2 != 0 ? this.m_attenuation : this.m_longAttenuation);
