...
// Type: Valve.VR.InteractionSystem.PlaySound
...
namespace Valve.VR.InteractionSystem
{
  ...
  public class PlaySound : MonoBehaviour
  {
    ...
    private void Awake()
    {
      ...
      if (this.audioSource.playOnAwake)
      {
        if (this.useRetriggerTime)
          this.InvokeRepeating("Play", this.timeInitial, Random.Range(this.timeMin, this.timeMax));
        else
          this.Play();
      }
    ...
    {
      if (this.looping)
        this.Invoke("PlayLooping", delayTime);
      else
        this.Invoke("PlayOneShotSound", delayTime);
    }
    ...
    public AudioClip PlayOneShotSound()
    {
      ...
      if (this.disableOnEnd)
        this.Invoke("Disable", this.clip.length);
    ...
    public AudioClip PlayLooping()
    {
      ...
      if (this.stopOnEnd)
        this.Invoke("Stop", this.audioSource.clip.length);
      return this.clip;
    }
    ...
    private void SetAudioSource()
    {
      if (this.useRandomVolume)
      {
        ...
        if (this.useRandomSilence && (double) Random.Range(0, 1) < (double) this.percentToNotPlay)
