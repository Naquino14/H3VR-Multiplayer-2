...
namespace FistVR
{
  public class MM_GrinderCrank : FVRInteractiveObject
  {
    public MM_Grinder Grinder;
    ...
    public override void UpdateInteraction(FVRViveHand hand)
    {
      ...
      float f = Mathf.Clamp(Mathf.Atan2(Vector3.Dot(this.YUpTarget.up, Vector3.Cross(forward, normalized)), Vector3.Dot(forward, normalized)) * 57.29578f, 0.0f, 10f);
      if ((double) f <= 0.0)
        return;
      this.Grinder.Crank(f);
      this.m_curCrankDir = Vector3.RotateTowards(this.m_curCrankDir, normalized, f * 0.0174533f, 0.0f);
